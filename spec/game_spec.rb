  ...
  RSpec.describe Game do
    ...
    context '#game_over?' do
      ...
      it 'should return hint 3' do
        expect(game.game_over?(hint).values[3]).to match(/[1-6]/)
      end

      it 'should return You have taken all hints.' do
        game.instance_variable_set(:@hint_count, 0)
        expect(game.game_over?(hint).values[3]).to eq 'You have taken all hints.'
      end
      ...
      context '#hint_answer' do
        it "should return 'You have taken all hints.'" do
          game.instance_variable_set(:@hint_count, 0)
          expect(game.send(:hint_answer)).to eq hint_answer
        end
